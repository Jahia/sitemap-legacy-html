...
<a href='<c:url value="${url.base}${currentNode.properties.base.node.path}.html"/>'>${currentNode.properties.base.node.properties["jcr:title"].string}
    level ${level}</a>
<c:forEach items="${jcr:getChildrenOfType(currentNode.properties.base.node,'jmix:sitemapLegacy')}" var="child" varStatus="childStatus">
    <c:if test="${childStatus.first}">
        <ul <c:if test="${level eq 1}"><c:set var="nbSubItems" value="${jcr:getChildrenOfType(currentNode.properties.base.node,'jmix:sitemapLegacy')}"/> id="primaryNav" class="col${fn:length(nbSubItems)}"</c:if>>
    </c:if>
